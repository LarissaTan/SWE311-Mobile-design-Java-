...
public class MyDatabase extends SQLiteOpenHelper {
    ...
    public static final String CREATE_USERDATA = "create table if not exists tb_UserData(userId integer primary key AUTOINCREMENT,name varchar(20),email varchar(50), age int, gender bool, location text, profilePicture int, foreign key (email) references tb_User(email));";
    ...
    */
    public static final String CREATE_COURSETYPE = "create table if not exists tb_CourseType(typeId integer primary key autoincrement, typeName varchar(20), level int);";
    // course table: store the video link for the child to learn
    public static final String CREATE_COURSE = "create table if not exists tb_Course(courseId integer primary key autoincrement, courseName varchar(50), typeId integer, videoID varchar(20), view int, foreign key(typeId) references tb_CourseType(typeId));";
    ...
    public static final String CREATE_GAME = "create table if not exists tb_Game(gameId integer primary key autoincrement, gameName varchar(50), typeId integer, foreign key(typeId) references tb_CourseType(typeId));";
    ...
    public static final String CREATE_USERGOAL = "create table if not exists tb_UserGoal(userId integer, typeId integer, date date, foreign key(userId) references tb_UserData(userId), foreign key(typeId) references tb_CourseType(typeId));";
    // user level: record the current level of the user (achievement)
    public static final String CREATE_USERLEVEL = "create table if not exists tb_UserLevel(userId integer, typeId integer, date date, foreign key(userId) references tb_UserData(userId), foreign key(typeId) references tb_CourseType(typeId));";
